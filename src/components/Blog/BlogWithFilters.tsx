...
const BlogWithFilters = () => {
  ...
  // Generate filter configuration from blog data
  const filterConfig = useMemo(() => generateFilterConfig(blogData, 'blog'), []);

  // Apply filters and sorting
  const filteredAndSortedBlogs = useMemo(() => {
    const filtered = filterContent(blogData, filters, 'blog');
    return sortContent(filtered, filters.sortBy, filters.sortOrder, 'blog');
  }, [filters]);
    ...
    <section className="pt-[120px] pb-[120px]">
      <div className="container">
        ...
        <ContentFilter
          ...
          filteredItems={filteredAndSortedBlogs.length}
          contentType="blog"
        />
        ...
        <div className={viewMode === 'list' ? 'space-y-0' : 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6'}>
          {filteredAndSortedBlogs.length > 0 ? (
            filteredAndSortedBlogs.map((blog) => (
              ...
              >
                <SingleBlog blog={blog} viewMode={viewMode} />
              </div>
            ))
          ) : (
            <div className={viewMode === 'list' ? 'w-full text-center py-16' : 'col-span-full text-center py-16'}>
              <div className="mx-auto max-w-md">
                ...
                <p className="text-body-color dark:text-body-color-dark">
                  Try adjusting your filters or search terms to find more blog posts.
                </p>
              </div>
            </div>
          )}
